...
/*** insert code***/
$('#pauta').click(function() {
  if ($(this).is(':checked')) {
    $("#pauta").parent("div").children("label").removeClass("error");
  }
});
    ...
    $('form').on('submit', function(e){
	   ...
	   if (!$("#pauta").is(":checked")) {
        ...
        if(error==0&&blockForm){
            blockForm=false;
            $.ajax({ url: 'process.php', type: 'POST', dataType: 'json', data: f.serializeArray(),complete: function(){ } })
                .done(function(data) {
                    if(data.rpta==='ok') {
                      blockForm=true;
                        ...
                        $('.gracias').fadeIn(350);
                        dataLayer.push({
                          'title': 'Programa Niños',
                          'page': '/programa-ninos/gracias',
                          'event': 'gracias'
                        });
                    }
                });
